      ...
      if(!empty($id) && is_numeric($id)){
          include("connexion.php");
             $date_archive = date('y-m-d');
              $list = "UPDATE utilisateurs SET Etat = 0 and dateArchive = '$date_archive' WHERE id=$id";
              $pdo->query($list);
...
$sql = 'SELECT COUNT(*) AS nb_utilisateurs FROM `utilisateurs` where Etat=1';
...
$sql = 'SELECT * FROM `utilisateurs` where Etat=1 ORDER BY `date` DESC LIMIT :premier, :parpage;';
...
<html>
 ...
 <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/css/bootstrap.min.css" rel="stylesheet">
  ...
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/js/bootstrap.bundle.min.js"></script>
   ...
   <span id="emar">
      <span>
      <a href="admin.php?" id="emp">  <h>liste des employers </h> </a>
      </span>
      <span>
       <a href=" "id="desar" ><h >liste archivé</h></a>
       </span>
<span>
<a href="deconnexion.php"> <img src="image/login_FILL0_wght400_GRAD0_opsz48.svg" alt=""></a> 
</span>
</span>
</header>
  <nav> 
  ...
  <div class="table-responsive">
    <table class="table table-bordered">
      ...
      <?php 
      include ("connexion.php");
      //$req=$pdo->prepare("select * from utilisateurs where Etat=1");
      //$req->execute();
      if (isset($_POST["recherche"])) {
         $rech= $_POST["recherche"];
         if (!empty($rech)) {
     
           $req = "SELECT * FROM utilisateurs WHERE Etat=1 AND nom LIKE '%$rech%' OR prenom LIKE '%$rech%'  ";
            $req = $pdo->prepare($req);
           $req->execute();
         }}
      while($row=$req->fetch())
      {
         ...
         <td id="action">
         <a href="?id_ar='.$id.'">
         <img id="archive" src="image/desarchive.png" alt="">
         </a>
